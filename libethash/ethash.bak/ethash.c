...
uint32_t EthCalcEpochNumber(uint8_t *SeedHash)
{
  ...
  LOG("C: ethash.c: Error on epoch calculation.");
...
void ethash_regenhash(struct work *work)
{
  ...
  LOG("C. ethash.c: Regenhash: First qword of input: 0x%016llX.", work->Nonce);
  ...
  LOG("C: ethash.c: Regenhash result: %s.", DbgHash);
  LOG("C. ethash.c: Last ulong: 0x%016llX.", bswap_64(*((uint64_t *)(work->hash + 0))));
  free(DbgHash);
}
